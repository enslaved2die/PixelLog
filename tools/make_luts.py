...
def write_3d_preview_lut(out_path, p):
    size = 33
    os.makedirs(os.path.dirname(os.path.abspath(out_path)), exist_ok=True)
    with open(out_path, "w") as f:
        f.write("# PixelLog to Rec.709 Preview 3D LUT\n")
        f.write(f"# Input: Pixel-Log BT.2020 [0, 1] -> Inverse -> BT.2020 to Rec.709 -> Filmic S-curve -> BT.709 OETF\n")
        ...
        for b_idx in range(size):
            ...
            for g_idx in range(size):
                ...
                for r_idx in range(size):
                    ...
                    # Filmic tone mapping on luminance
    ...
    print(f"[make_luts] Generated 3D Preview LUT: {out_path}")
...
def main():
    ...
    asset_lut = os.path.join(root_dir, "app/src/main/assets/luts/PixelLog_to_Rec709_Display.cube")
    ...
    write_dctl(os.path.join(post_dir, "PixelLog_Transform.dctl"), p) # Keep backward compatibility

    preview_lut_path = os.path.join(post_dir, "PixelLog_to_Rec709_65.cube")
    write_3d_preview_lut(preview_lut_path, p)
    shutil.copyfile(preview_lut_path, asset_lut)
    print(f"[make_luts] Updated asset display LUT: {asset_lut}")
